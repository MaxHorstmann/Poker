...
namespace Poker.Common
{
    ...
    public class LayoutAwarePage : Page
    {
        ...
        protected virtual void LoadState(Object navigationParameter, Dictionary<String, Object> pageState)
        {
            ...
            Table table = null;
            if (pageState!=null && pageState.ContainsKey(Constants.SETTINGS_TABLE))
            {
                try
                {
                    table = pageState[Constants.SETTINGS_TABLE] as Table;
                    myApp.CurrentTable = table;
                }
                catch (Exception) // so many things can go wrong. no matter what it is....
                {
                    table = null; // ...something went wrong, we couldn't restore the state
                }
            }

            //// HandHistory
            //if (_table.HandHistory == null)
            //{
            //    _table.HandHistory = new HandHistoryWindowsStoreApp();
            //}

            //_table.PlayerHasLeft = false;
        ...
        {
            App myApp = App.Current as App;
            Table _table = myApp.CurrentTable as Table;
            if (_table != null)
            {
                pageState.Add(Constants.SETTINGS_TABLE, _table);
            }
        }
